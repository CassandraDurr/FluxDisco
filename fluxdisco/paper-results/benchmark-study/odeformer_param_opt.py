"""Parameter optimisation for ODEFormer predictions from ODEFormer repository."""

# ------------------------------------------------------------------------------------
# Code from https://github.com/sdascoli/odeformer/blob/main/param_optimizer.py
...
class ConstantOptimizer(PredictionIntegrationMixin):
    ...
    def get_params(self) -> Dict[str, float]:
        ...
        def replace_constant_by_param(
            match: regex.Match, param_prior: Dict[str, float]
        ) -> str:
    ...
    def objective(self, params: np.ndarray) -> float:
        ...
        if self.track_eval_history:
            self.eval_history.append(
                [self._objective(simulated_trajectory, self.eval_objective), *params]
            )
    ...
    def _objective(self, simulated_trajectory, objective: str):
        ...
        try:
            if objective == "r2":
                return -1 * variance_weighted_r2_score(
                    self.observed_trajectory, simulated_trajectory
                )
            elif objective == "mse":
    ...
    def optimize(self) -> Tuple[str, np.ndarray, np.ndarray]:
        ...
        info = minimize(
            ...
            x0=(
                np.random.randn(len(param_priors)) if self.init_random else param_priors
            ),
        )
        if self.track_eval_history:
            # take best params
        ...
        return (
            self.insert_params(optimal_params),
            optimal_params,
            self.simulate(optimal_params),
        )
...
def main(args, result_dir, result_file):
    ...
    for samples_i, (samples, _) in enumerate(tqdm(pmlb_iterator)):
        ...
        for _trajectory_i, (_times, _trajectory) in enumerate(zip(times, trajectories)):
            ...
            pred_eq = " | ".join(
                [str(sympy.parse_expr(e)) for e in scores.predicted_trees.split("|")]
            )
            ...
            final_eq, estimated_params, simulated_trajectory = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path_scores", type=str, default="./experiments/odeformer/scores.csv"
    )
    ...
    if args.init_random:
        result_dir = (
            Path(args.path_scores).parent
            / "optimize_init_random"
