"""Base estimation code from the Symbolic Physics Learner repository."""

# ------------------------------------------------------------------------------------
# Code from https://github.com/isds-neu/SymbolicPhysicsLearner/blob/main/spl_base.py
...
class SplBase:
    ...
    def __init__(
        ...
        self.grammars = base_grammars + [
            x for x in aug_grammars if x not in base_grammars
        ]
    ...
    def get_unvisited(self, state, node):
        ...
        return [
            a for a in valid_action if self.QN[state + "," + self.grammars[a]][1] == 0
        ]
    ...
    def step(self, state, action_idx, ntn):
        ...
        if not ntn:
            reward, eq = self.score(
                self.tree_to_eq(state.split(",")),
                len(state.split(",")),
                self.data_sample,
                eta=self.eta,
            )
    ...
    def update_ucb_mcts(self, state, action):
        ...
        return Q_child / N_child + self.exploration_rate * np.sqrt(
            np.log(N_parent) / N_child
        )
    ...
    def backpropogate(self, state, action_index, reward):
        ...
        while state:
            if self.scale != 0:
                ...
                self.QN[state][0] += 0
            self.QN[state][1] += 1
            self.UCBs[state][self.grammars.index(action)] = self.update_ucb_mcts(
                state, action
            )
            if "," in state:
        ...
        if state.count(",") <= self.max_module:
            if not self.good_modules:
                ...
                if len(self.good_modules) < self.max_aug:
                    self.good_modules = sorted(
                        self.good_modules + [(module, reward, eq)], key=lambda x: x[1]
                    )
                else:
                    if reward > self.good_modules[0][1]:
                        self.good_modules = sorted(
                            self.good_modules[1:] + [(module, reward, eq)],
                            key=lambda x: x[1],
                        )
    ...
    def run(self, num_episodes, num_play=50, print_flag=False, print_freq=100):
        ...
        for i_episode in range(1, num_episodes + 1):
            if (i_episode) % print_freq == 0 and print_flag:
                print(
                    "\rEpisode {}/{}, current best reward {}.".format(
                        i_episode, num_episodes, best_solution[1]
                    ),
                    end="",
                )
                sys.stdout.flush()
